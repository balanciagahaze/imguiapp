...
enum ImGuiKey : int;               // -> enum ImGuiKey_             // Enum: A key identifier (ImGui-side enum)
...
namespace ImGui
{
    ...
    IMGUI_API void          StyleColorsOrange(ImGuiStyle* dst = NULL); // classic imgui style
...
struct ImDrawList
{
    ...
    IMGUI_API void  AddText2(const ImVec2& pos, ImU32 col, const char* text_begin, const char* text_end = NULL);
    IMGUI_API void  AddText2(const ImFont* font, float font_size, const ImVec2& pos, ImU32 col, const char* text_begin, const char* text_end = NULL, float wrap_width = 0.0f, const ImVec4* cpu_fine_clip_rect = NULL);
...
struct ImFontGlyphRangesBuilder
{
    ...
    IMGUI_API void  AddText2(const char* text, const char* text_end = NULL);     // Add string (each character of the UTF-8 string are added)
